...
class CosineTest extends FunSuite{
	...
	val iModel=new Model(2,"italian")
	...
	val jaModel=Assimilator.assimilateURL("https://ja.wikipedia.org/wiki/ナポレオン・ボナパルト", new Model(2,"japanese"))
			val chModel=Assimilator.assimilateURL("https://zh.wikipedia.org/wiki/拿破仑一世", new Model(2,"chinese"))
			...
			val arModel=Assimilator.assimilateURL("https://ar.wikipedia.org/wiki/%D9%86%D8%A7%D8%A8%D9%84%D9%8A%D9%88%D9%86_%D8%A7%D9%84%D8%A3%D9%88%D9%84", new Model(2,"arabic"))
			test("recognize with the recognizer") {
			val r=new Recognizer(List(gModel,iModel,jaModel,chModel))
			r+=arModel
			assert("japanese"==r.identifiedLanguage(Assimilator.assimilateString("来るものは拒まず去るものは追わず", new Model(2,"japanese proverb"))))
			assert("italian"==r.identifiedLanguage(Assimilator.assimilateString("Tanto va la gatta al lardo che ci lascia lo zampino", new Model(2,"italian proverb"))))
			assert("chinese"==r.identifiedLanguage(Assimilator.assimilateString("今川義元掌權後，不斷於與北條家、織田家爭權，而且勢力不斷擴大，並且於1554年訂立了今川家、武田家與北條家的同盟，史稱善德­寺三國同盟", new Model(2,"chinese statement"))))
	}
}

class ModelLoadSaveTest extends FunSuite{
	test("save a model") {
